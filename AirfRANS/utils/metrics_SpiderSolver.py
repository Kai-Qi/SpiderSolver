...
def Infer_test(device, models, hparams, data, coef_norm=None):
    ...
    while cond:
        ...
        # Cumulative indices for surface & spatial points
        surf_mask = data_sampled.surf[idx] 
        idx_surf = idx[surf_mask]
        idx_velo = idx[~surf_mask]
        # 1. Cumulative summation
        cumsum_surf = torch.cumsum(data_sampled.surf, dim=0)  # Surface points
        cumsum_velo = torch.cumsum(~data_sampled.surf, dim=0) # patial points
        # 2. Split indices by type
        index_surf = cumsum_surf[idx_surf]-1
        index_velo = cumsum_velo[idx_velo]-1
            
        surf_cluster_result = data_sampled.labels_SpectralClustering[0]
        closest_indices_on_surface = data_sampled.closest_indices_on_surface[0]       
        velo_cluster_index = surf_cluster_result[closest_indices_on_surface]
        velo_cluster_index = velo_cluster_index[index_velo]
        velo_cluster_index = torch.tensor(velo_cluster_index)
        ...
        data_sampled.onion_index = data_sampled.onion_index[:,index_velo]
        data_sampled.velo_cluster_index = velo_cluster_index
        data_sampled.labels_SpectralClustering[0] = data_sampled.labels_SpectralClustering[0][index_surf]
        data_sampled.onion_index_0 = data_sampled.onion_index_0[index_velo]
...
def Results_test(template, labels_SpectralClustering, device, models, hparams, coef_norm, path_in, path_out, n_test=3, criterion='MSE', x_bl=[.2, .4, .6, .8], s='full_test', file=None, onion_num = 14, n_clusters= 4):
    '''
    ...
    with open(osp.join(path_in+ file + '.json'), 'r') as f:
        manifest = json.load(f)

    test_dataset = manifest[s]
    print(len(manifest[s]))

    idx = random.sample(range(len(test_dataset)), k=n_test)
    idx.sort()

    test_dataset_vtk = Dataset(test_dataset, sample=None, coef_norm=coef_norm, my_path=path_in, onion_num = onion_num, n_clusters= n_clusters)
    
    for i in range(len(test_dataset_vtk)):
        surf = test_dataset_vtk[i]['surf']
        point = test_dataset_vtk[i]['x'][surf,0:2].numpy()
        index = test_dataset_vtk[i]['index']
        ...
        # Vote on neighbor labels for each row
        def mode(row):
        ...
        test_dataset_vtk[i]['labels_SpectralClustering'] = most_frequent
        test_dataset_vtk[i]['index'] = index
    ...
    return true_coefs, pred_coefs_mean, pred_coefs_std, true_surf_coefs, surf_coefs, true_bls, bls, \
           scores_vol.mean(axis=0)[0,2], scores_surf.mean(axis=0)[0,2], scores_force.mean(axis=0)[0,1], spear_coefs.mean(axis=0)[0,1]
