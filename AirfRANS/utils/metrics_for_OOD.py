...
def Infer_test(device, models, hparams, data, coef_norm=None):
    ...
    while cond:
        ...
        # 累积求和
        cumsum_surf = torch.cumsum(data_sampled.surf, dim=0)  # 表面点的累计和
        cumsum_velo = torch.cumsum(~data_sampled.surf, dim=0)  # 空间点的累计和
        # 将表面点和空间点的索引分开
...
def Results_test(device, models, hparams, coef_norm, path_in, path_out, n_test=3, criterion='MSE',
                 x_bl=[.2, .4, .6, .8], s='full_test', file=None):
    '''
    ...
    for i in range(len(models[0])):
        ...
        avg_loss_vol_var = np.zeros((len(model), 2))
        ...
        for j, data in enumerate(tqdm(test_loader)):
            ...
            for n, out in enumerate(outs):
                ...
                loss_surf_var = criterion(out[data.surf, 1], data.y[data.surf, 1]).mean(dim=0)
                loss_vol_var = criterion(out[data.surf, 0:2], data.y[data.surf, 0:2]).mean(dim=0)
