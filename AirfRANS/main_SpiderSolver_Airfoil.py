...
template = compute_template(train_dataset)
# Perform spectral clustering on template shape
labels_SpectralClustering = perform_spectral_clustering(template, args.n_clusters)
# Save results and plot
save_template_and_plot(template, labels_SpectralClustering, path, current_file_path)

# Change clustering results  
process_dataset(train_dataset, template, labels_SpectralClustering, path)
process_dataset(val_dataset, template, labels_SpectralClustering, path)
...
model = train_clip.main(device, train_dataset, val_dataset, model, hparams, log_path,
                criterion='MSE_weighted', val_iter=10, reg=args.weight, name_mod=args.model, val_sample=True, clip = args.clip)
print('end training')

models.append(model)
torch.save(models, osp.join(path, args.model))
