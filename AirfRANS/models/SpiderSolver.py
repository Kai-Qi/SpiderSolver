...
class SpiderSolver_Attention_1D(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0., onion_num=14, n_clusters = 4):
        super().__init__()
        ...
        self.weight_velo_car = nn.Linear(dim, onion_num*self.surf_clustering_num)
        self.weight_velo_velo = nn.Linear(dim, onion_num*self.surf_clustering_num)
    ...
    def _process_coarse_attention(self, x, cfd_data, B, C):
        ...
        velo_first_lap = velo_fx_mid[:, cfd_data.onion_index_0.bool(), :]
    ...
    def _process_fine_attention(self, x, cfd_data, B):
        ...
        velo_cluster_index = cfd_data.velo_cluster_index
        
        surf_fx_mid = fx_mid[:, cfd_data.surf, :]
        velo_fx_mid = fx_mid[:, ~cfd_data.surf, :]
        velo_total_num = velo_fx_mid.shape[1]
        ...
        velo_tokens_index = torch.zeros(1, 1, self.surf_clustering_num, velo_total_num, 1).cuda()
    ...
    def _process_t2p_operation(self, x, cfd_data, B, C, num, velo_first_lap, surf_fx_mid_first_lap, out_slice_token):
        ...
        feature_velo[:, cfd_data.onion_index_0.bool(), :] = \
            feature_velo[:, cfd_data.onion_index_0.bool(), :] + velo_first_lap
    ...
    def forward(self, x, cfd_data):
        ...
        B, N, C = x.shape
        num = self.onion_num * self.surf_clustering_num
        ...
        new_feature = self._process_t2p_operation(x, cfd_data, B, C, num, velo_first_lap, 
...
class SpiderSolver_block(nn.Module):
    ...
    def __init__(
        ...
        self.Attn = SpiderSolver_Attention_1D(hidden_dim, heads=num_heads, dim_head=hidden_dim // num_heads,
                                         dropout=dropout, onion_num=onion_num, n_clusters= n_clusters)
...
class SpiderSolver(nn.Module):
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([SpiderSolver_block(num_heads=n_head, hidden_dim=n_hidden,
                                                      ...
                                                      last_layer=(_ == n_layers - 1), onion_num= onion_num, n_clusters =n_clusters)
