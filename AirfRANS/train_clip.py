...
def train_clip(device, model, train_loader, optimizer, scheduler, criterion='MSE', reg=1, clip = 50):
    model.train()
    ...
    for data in train_loader:
        ...
        norm10.append( nn.utils.clip_grad_norm_(model.parameters(), max_norm=clip, norm_type=2) )
    ...
    return avg_loss.cpu().data.numpy() / iter, avg_loss_per_var.cpu().data.numpy() / iter, avg_loss_surf_var.cpu().data.numpy() / iter, avg_loss_vol_var.cpu().data.numpy() / iter, \
           avg_loss_surf.cpu().data.numpy() / iter, avg_loss_vol.cpu().data.numpy() / iter, norm10
...
def main(device, train_dataset, val_dataset, Net, hparams, path, criterion='MSE', reg=1, val_iter=10,
         name_mod='GraphSAGE', val_sample=True, clip = 50):
    '''
        Args:
    ...
    for epoch in range(hparams['nb_epochs']):
        ...
        for data in train_dataset:
            ...
            cumsum_surf = torch.cumsum(data_sampled.surf, dim=0)  
            cumsum_velo = torch.cumsum(~data_sampled.surf, dim=0)  
            # 2. Split indices by type
            index_surf = cumsum_surf[idx_surf]-1
            index_velo = cumsum_velo[idx_velo]-1

            surf_cluster_result = data_sampled.labels_SpectralClustering
            
            closest_indices_on_surface = data_sampled.closest_indices_on_surface       
            ...
            data_sampled.labels_SpectralClustering = data_sampled.labels_SpectralClustering[index_surf]
        ...
        if epoch < 10:
            train_loss, _, loss_surf_var, loss_vol_var, loss_surf, loss_vol, norm10 = train_clip(device, model, train_loader, optimizer, lr_scheduler, criterion, reg=reg, clip = 1e6)
        if epoch >= 10:
            train_loss, _, loss_surf_var, loss_vol_var, loss_surf, loss_vol, norm10 = train_clip(device, model, train_loader, optimizer, lr_scheduler, criterion, reg = reg, clip = clip)
        ...
        t2 = default_timer()
        print(epoch, '%.2f'% (t2-t1), 'train_loss:', '%.4f'% train_loss, 'loss_surf:', '%.4f'% loss_surf, 'val_loss:', '%.4f'% val_loss, 'val_surf:', '%.4f'% val_surf,
