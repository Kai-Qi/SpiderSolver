...
def generate_onion_indices_and_closest_points(x, surf, pos_surf, pos_ext, onion_num):
    """
    Generate onion indices based on SDF sorting and find closest surface points
    Args:
        x: Input tensor containing SDF values
        surf: Surface boolean mask
        pos_surf: Surface point positions
        pos_ext: External point positions
        onion_num: Number of onion layers
    Returns:
        onion_index: Onion layer indices tensor
        closest_indices_on_surface: Indices of closest surface points
    """
    # Extract SDF values for non-surface points
    sdf = x[~surf.bool(), 3]
    sorted_sdf, sorted_indices = torch.sort(sdf, descending=False, dim=-1)  
    
    # Calculate point distribution for onion layers
    point_num_list_1 = np.ones(1) * 500
    point_num_list_2 = np.ones(onion_num - 2) * int(28004 / (onion_num - 1))
    point_num_list_3 = np.ones(1) * (28004 - (onion_num - 2) * (int(28004 / (onion_num - 1))))
    ...
    # Assign points to onion layers based on sorted SDF
    counter = 0 
    ...
    # Assign remaining points to the last layer
    zeros = torch.zeros(sdf.shape[0])
    zeros[sorted_indices[counter:]] = 1.0
    onion_index[onion_slice_num - 1, :] = zeros
    
    # Find closest surface points for external points
    surface_points = np.concatenate((pos_surf[0:16, :], pos_surf[112:, :]), axis=0) 
    distances = cdist(pos_ext, surface_points, metric='euclidean')
    closest_indices_on_surface = np.argmin(distances, axis=1)
    
    return onion_index, closest_indices_on_surface
...
def get_datalist(root, samples, norm=False, coef_norm=None, savedir=None, preprocessed=False, onion_num = 15, surf_clustering_num = 6):
    dataset = []
    ...
    for k, s in enumerate(samples):
        if preprocessed and savedir is not None:
            ...
            # different from Transolver model
            points_press_3586 = np.concatenate((points_press[0:16,:], points_press[112:,:]), axis=0)
            
            sdf_velo, normal_velo = get_sdf(points_velo, points_press_3586)
        ...
        onion_index, closest_indices_on_surface = generate_onion_indices_and_closest_points(
            x=x,
            surf=surf,
            pos_surf=pos_surf,
            pos_ext=pos_ext,
        ...
        data = Data(pos=pos,x=x, y=y, surf=surf.bool(), \
                    ...
                    closest_indices_on_surface = closest_indices_on_surface, labels_SpectralClustering = None)                 
        dataset.append(data)
...
if __name__ == '__main__':
    ...
    root = '/home/qikai/mlcfd_data/training_data'
    save_path = '/home/qikai/mlcfd_data/preprocessed_data/param0/' + file_name
