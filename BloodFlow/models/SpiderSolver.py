...
class SpiderSolver_Attention_1D(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0., n_clusters = 4, onion_num = 6):
        super().__init__()
        ...
        self.surf_clustering_num = n_clusters
        self.onion_num = onion_num
        
        # self.in_project_x_surf_clustering = SurfacePointClassifier(input_dim=inner_dim,num_classes=self.surf_clustering_num)

        self.in_project_x = nn.Linear(dim, inner_dim)
        self.in_project_fx = nn.Linear(dim, inner_dim)
        
        self.weight_surf = nn.Linear(dim, 1)
        self.weight_velo_car = nn.Linear(dim, self.surf_clustering_num*self.onion_num)

        self.weight_velo_1 = nn.Linear(dim, self.surf_clustering_num*self.onion_num)
        # self.weight_velo_2 = nn.Linear(dim, 14)
        # self.weight_velo_3 = nn.Linear(dim, 14)
        # self.weight_velo_4 = nn.Linear(dim, 14)

        
        for l in [self.weight_surf]:
            torch.nn.init.orthogonal_(l.weight)  # use a principled initialization
        
        for l in [self.weight_velo_1]:
            torch.nn.init.orthogonal_(l.weight)  # use a principled initialization
    ...
    def _process_coarse_attention(self, x, cfd_data, B, C):
        ...
        velo_first_lap = fx_mid[:, cfd_data.onion_index_0.bool(), :]
        first_lap_tokens = torch.cat([velo_first_lap, surf_fx_mid], axis=1)
        num_velo = velo_first_lap.shape[1]
        ...
        # Reshape outputs
        velo_first_lap = out_slice_token_first_lap[:, :, 0:num_velo, :].permute(0, 2, 1, 3)
        velo_first_lap = velo_first_lap.reshape(B, velo_first_lap.shape[1], velo_first_lap.shape[2] * velo_first_lap.shape[3])
        
        surf_fx_mid_first_lap = out_slice_token_first_lap[:, :, num_velo:, :].permute(0, 2, 1, 3)
    ...
    def _process_fine_attention(self, x, cfd_data, B):
        ...
        velo_cluster_index = cfd_data.velo_cluster_index_one_hot[None, None, :, :, None]
        new_velo_tokens = velo_tokens[:, :, :, ~cfd_data.surf, :] * velo_cluster_index
        ...
        surf_tokens = torch.mean(surf_fx_mid_new, axis=1, keepdim=True)
        ...
        # Compute fine attention among tokens
    ...
    def _process_t2p_operation(self, x, cfd_data, B, C, num, velo_first_lap, surf_fx_mid_first_lap, out_slice_token):
        """Process T2P (Token to Point) operation"""
        ...
        out_slice_surf = out_slice_token[:, :, 0, :].reshape(B, 1, C)
        feature_surf = weight_surf @ out_slice_surf
        
        # Process velocity features
        out_slice_velo = out_slice_token[:, :, 1:, :].permute(0, 2, 1, 3).reshape(B, num, C)
        weight_velo_1 = self.weight_velo_1(velo_x_mid)
        feature_velo = weight_velo_1 @ out_slice_velo
        ...
        # Fuse velocity features with coarse attention results
        new_feature[:, cfd_data.onion_index_0.bool(), :] = \
            new_feature[:, cfd_data.onion_index_0.bool(), :] + velo_first_lap

        new_feature = self.to_out(new_feature)
        return new_feature
    
    
    def forward(self, x, cfd_data):
        """
        Main forward pass
        Args:
            x: Input tensor of shape [B, N, C]
            cfd_data: CFD data containing surface masks, onion indices, etc.
        Returns:
            new_feature: Processed output tensor of shape [B, N, C]
        """
        # B N C
        B, N, C = x.shape
        ...
        out_slice_token = self._process_fine_attention(x, cfd_data, B)

        ### (3) T2P operation: From Token to Point-Wise Features
        new_feature = self._process_t2p_operation(x, cfd_data, B, C, num, velo_first_lap, surf_fx_mid_first_lap, out_slice_token)
...
class SpiderSolver_block(nn.Module):
    ...
    def __init__(
        ...
        self.Attn = SpiderSolver_Attention_1D(hidden_dim, heads=num_heads, dim_head=hidden_dim // num_heads,
                                         dropout=dropout, n_clusters = n_clusters, onion_num = onion_num)
    ...
    def forward(self, fx, data):
        fx = self.Attn(self.ln_1(fx), data) + fx
...
class SpiderSolver(nn.Module):
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([SpiderSolver_block(num_heads=n_head, hidden_dim=n_hidden,
                                                      ...
                                                      last_layer=(_ == n_layers - 1), n_clusters= n_clusters, onion_num = onion_num)
